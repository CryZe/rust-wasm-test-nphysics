...
extern "C" {
    ...
    pub fn emscripten_set_main_loop(m: extern "C" fn(), fps: libc::c_int, infinite: libc::c_int);
}
...
extern "C" fn rust_caller<F: FnMut(Event)>(a: *const libc::c_void,
                                           docptr: *const libc::c_void,
                                           id: i32) {
...
impl<'a> HtmlNode<'a> {
    ...
    pub fn data_get(&self, s: &str) -> Option<String> {
        ...
        if a == -1 {
            ...
            Some(unsafe {
                str::from_utf8(CStr::from_ptr(a as *const libc::c_char).to_bytes())
                    .unwrap()
                    .to_owned()
            })
        }
    ...
    pub fn set_style(&self, v: &str) {
        js! { (self.id, v) b"\
            WEBPLATFORM.rs_refs[$0].style.cssText = UTF8ToString($1);\
        \0" };
    }
...
pub fn init<'a>() -> Document<'a> {
    ...
    \0" };
    Document { refs: Rc::new(RefCell::new(Vec::new())) }
}

extern "C" fn leavemebe() {
    unsafe {
